...
const API_POST_ROUTES = {
  ...
  // Records where a governed run reached. It is not a restore and not a
  // release: the canonical authority named below creates no commit, and
  // rollback stays deferred to its own packet. Every checkpoint prerequisite
  // is decided there, under its own claim, from the run id and nothing else.
...
const DEFAULT_CONTROL_AUTHORITIES = Object.freeze({
  ...
  checkpointRun: AegisRun.checkpointRun,
});
...
const CONTROL_AUTHORITY_NAMES = Object.freeze(
  ['bindIndependentReview', 'checkpointRun', 'prepareIndependentReview',
...
function resolveControlAuthorities(candidate) {
  ...
  if (keys.length !== CONTROL_AUTHORITY_NAMES.length ||
    ...
    throw new TypeError(
      ...
      'prepareIndependentReview, requestIndependentReview and checkpointRun');
  }
  return Object.freeze({
    ...
    checkpointRun: descriptors.checkpointRun.value,
  });
}
...
// ── one authenticated checkpoint ────────────────────────────────────────────
// A checkpoint RECORDS where a governed run reached. It restores nothing and
// deploys nothing: rollback is deferred to its own packet, and the canonical
// authority creates no commit — a run whose reviewed bytes are still
// uncommitted is refused there exactly as it is on the CLI.
//
// Every prerequisite stays where it already lives: REVIEW_BOUND, deterministic
// checks that actually ran and passed, the step-9 watchdog, one exact evidence
// chain across review/checks/packet/subject, a clean tree whose HEAD already
// contains the reviewed subject, and the per-run claim. This route owns the
// HTTP transport and nothing else. It takes no claim, writes no record, starts
// no process, retries nothing and cannot override a prerequisite, because the
// only coordinate it may send is a runId this host validated — the shared
// runId-only parser refuses a second key, so a caller cannot post a commit, a
// packet, a force flag, a rollback target or a subject.
//
// The canonical answer carries more than a browser may read: the packet path
// and digest, the reviewed subject's paths, ranges and byte count, the review
// coordinates and the run objective. None of it is spread onto the response.
// The DTO below is CONSTRUCTED from coordinates that each pass a format check
// first, so a field the callable adds later cannot travel by accident.
const CHECKPOINT_ACTION = 'checkpoint';
const CHECKPOINT_RUN_ID_RE = /^RUN-\d{8}-[0-9a-f]{8}$/;
const CHECKPOINT_ID_RE = /^CP-\d{14}-[0-9a-f]{8}$/;
const CHECKPOINT_COMMITISH_RE = /^[0-9a-f]{40,64}$/;

// Every sentence a browser can read is written HERE. The canonical refusal
// messages are generated by concatenating gate rules, run state and watchdog
// problems, so the CODE travels and the upstream TEXT never does. None of
// these say what did or did not happen to the run: they say what was refused.
const PUBLIC_CHECKPOINT_REFUSALS = Object.freeze({
  INVALID_RUN_ID: Object.freeze({ httpStatus: 400,
    message: 'runId is not a canonical AEGIS run identifier.' }),
  RUN_NOT_FOUND: Object.freeze({ httpStatus: 404,
    message: 'No run with that identifier exists.' }),
  INVALID_CHECKPOINT: Object.freeze({ httpStatus: 409,
    message: 'This run has not reached the state a checkpoint is recorded from; independent review binds first.' }),
  CHECKPOINT_UNPROVEN: Object.freeze({ httpStatus: 409,
    message: 'A checkpoint requires deterministic checks that actually ran and passed on this run.' }),
  CHECKPOINT_WATCHDOG_REFUSED: Object.freeze({ httpStatus: 409,
    message: 'The canonical watchdog refuses a checkpoint on this run because a required stage was skipped.' }),
  CHECKPOINT_EVIDENCE_INVALID: Object.freeze({ httpStatus: 409,
    message: 'The review, the deterministic checks, the packet and the subject do not form one exact evidence chain.' }),
  CHECKPOINT_DIRTY_TREE: Object.freeze({ httpStatus: 409,
    message: 'The reviewed subject is not committed. This route creates no commit; the approved external narrow-commit path commits exactly that subject first.' }),
  CHECKPOINT_NO_ROLLBACK_POINT: Object.freeze({ httpStatus: 409,
    message: 'No commit could be resolved as a point to return to.' }),
  CHECKPOINT_HEAD_UNRELATED: Object.freeze({ httpStatus: 409,
    message: 'The current commit is not a descendant commit containing the reviewed subject.' }),
  CHECKPOINT_SUBJECT_MISMATCH: Object.freeze({ httpStatus: 409,
    message: 'The current committed version does not contain exactly the checked and reviewed subject.' }),
  CHECKPOINT_TREE_INVALID: Object.freeze({ httpStatus: 409,
    message: 'The tree of the current commit could not be resolved.' }),
});

// Transport completion is not an outcome. These two words are the only ones
// this route may publish, and the second one is deliberately not a failure:
// an unreadable answer means this host does not KNOW, and reporting "nothing
// happened" would be a fabricated no-mutation claim.
const PUBLIC_CHECKPOINT_SUMMARIES = Object.freeze({
  RECORDED:
    'A checkpoint was recorded for this run. It marks a known-good point and nothing else: it restores no files and deploys nothing.',
  UNCERTAIN:
    'This host asked the canonical checkpoint authority once and could not read what came back. Whether a checkpoint was recorded is unknown — treat this run as neither checkpointed nor unchanged until the canonical run record is read. A checkpoint never restores files and never deploys.',
});

const PUBLIC_CHECKPOINT_UNCERTAIN_REASONS = Object.freeze({
  CHECKPOINT_CLAIM_NOT_RELEASED:
    'The canonical authority reports that a checkpoint record was written for this run but could not prove it released the per-run claim it took. This run is not idle, and it is not safely checkpointed, until that claim is inspected.',
  CHECKPOINT_CALL_FAILED:
    'The canonical checkpoint call did not complete, so what it recorded before it stopped is unknown.',
  CHECKPOINT_RESULT_UNREADABLE:
    'The canonical checkpoint call produced no answer this host recognises, so what it recorded is unknown.',
});

function publicCheckpoint(runId, fields) {
  return {
    runId,
    ...
    rollbackPoint: fields.rollbackPoint,
    tree: fields.tree,
    digest: fields.digest,
    reasonCode: fields.reasonCode,
    reasonSummary: fields.reasonCode ? PUBLIC_CHECKPOINT_UNCERTAIN_REASONS[fields.reasonCode] : null,
    summary: PUBLIC_CHECKPOINT_SUMMARIES[fields.outcome],
  };
}

// One fixed shape for every outcome this route cannot read, so a browser panel
// never has to tell "no field" apart from "unknown field". It never publishes
// a checkpoint id, a commit or a state, because it has none it trusts.
function uncertainCheckpoint(runId, reasonCode) {
  return publicCheckpoint(runId, {
    outcome: 'UNCERTAIN', state: null, checkpointId: null, createdAt: null,
    rollbackPoint: null, tree: null, digest: null, reasonCode,
  });
}

/**
 * Build the public checkpoint DTO, or null when the canonical answer is not one
 * this host recognises. Null is not an empty answer: the caller turns it into
 * the uncertain outcome above, because publishing half-validated coordinates as
 * a recorded checkpoint is worse than saying the answer was unreadable.
 */
function minimizeCheckpoint(runId, answer) {
  if (!answer || typeof answer !== 'object' || Array.isArray(answer)) return null;
  // The answer must be about the run that was asked about, and it must be the
  // checkpoint action reporting the checkpointed state. Any mismatch is an
  // unrecognised answer, not a result to publish under this run's name.
  if (answer.runId !== runId || answer.state !== 'CHECKPOINTED' ||
      answer.action !== CHECKPOINT_ACTION) return null;
  const checkpointId = typeof answer.checkpointId === 'string' &&
    CHECKPOINT_ID_RE.test(answer.checkpointId) ? answer.checkpointId : null;
  const createdAt = publicTimestamp(answer.createdAt);
  const rollbackPoint = typeof answer.rollbackPoint === 'string' &&
    CHECKPOINT_COMMITISH_RE.test(answer.rollbackPoint) ? answer.rollbackPoint : null;
  const tree = typeof answer.tree === 'string' &&
    CHECKPOINT_COMMITISH_RE.test(answer.tree) ? answer.tree : null;
  const digest = publicSha256(answer.digest);
  if (!checkpointId || !createdAt || !rollbackPoint || !tree || !digest) return null;
  return publicCheckpoint(runId, {
    outcome: 'RECORDED', state: 'CHECKPOINTED', checkpointId, createdAt,
    rollbackPoint, tree, digest, reasonCode: null,
  });
}

function recordCheckpoint(runId, controlAuthorities) {
  if (!CHECKPOINT_RUN_ID_RE.test(runId)) {
    throw new AegisRun.AegisControlError('INVALID_RUN_ID',
      PUBLIC_CHECKPOINT_REFUSALS.INVALID_RUN_ID.message, 400);
  }
  let answer;
  try {
    // Exactly one canonical invocation, with exactly one coordinate: the runId
    // validated above. There is no second call and no override anywhere here.
    answer = controlAuthorities.checkpointRun(runId);
  } catch (error) {
    const mapped = error instanceof AegisRun.AegisControlError &&
      typeof error.code === 'string' &&
      Object.prototype.hasOwnProperty.call(PUBLIC_CHECKPOINT_REFUSALS, error.code)
      ? PUBLIC_CHECKPOINT_REFUSALS[error.code] : null;
    // A known refusal is raised by the canonical authority before it records
    // anything, so it travels as a refusal — under host-fixed wording, never
    // the canonical message, which is generated from paths, run state and
    // watchdog problems.
    if (mapped) throw new AegisRun.AegisControlError(error.code, mapped.message, mapped.httpStatus);
    // Everything else is uncertainty, not a refusal. A claim this process
    // could not prove it released, and any failure raised deeper than the
    // checkpoint gates, both leave what happened on disk unknown.
    return uncertainCheckpoint(runId,
      error instanceof AegisRun.AegisControlError && error.code === 'CHECKPOINT_CLAIM_NOT_RELEASED'
        ? 'CHECKPOINT_CLAIM_NOT_RELEASED' : 'CHECKPOINT_CALL_FAILED');
  }
  return minimizeCheckpoint(runId, answer) || uncertainCheckpoint(runId, 'CHECKPOINT_RESULT_UNREADABLE');
}
...
async function handleApi(req, res, config, pathname, ctx, started, controlAuthorities, launchWorker) {
  ...
  try {
    ...
    else if (pathname === '/api/checkpoint') result = recordCheckpoint(runId, controlAuthorities);
...
module.exports = {
  ...
  minimizeCheckpoint, PUBLIC_CHECKPOINT_REFUSALS, PUBLIC_CHECKPOINT_SUMMARIES,
  PUBLIC_CHECKPOINT_UNCERTAIN_REASONS,
};
